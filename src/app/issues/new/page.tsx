...
const workers = [
  { id: '1', name: '김기사', specialty: '배관, 전기' },
  { id: '2', name: '박배관', specialty: '배관' },
  { id: '3', name: '이청소', specialty: '청소' },
]
      ...
      <div className="flex-1 px-5">
        ...
        {step === 1 && (
          <div>
            ...
            {house && room && (
              <button onClick={() => setStep(2)} className="w-full mt-6 py-3.5 rounded-xl text-[15px] font-semibold bg-[var(--blue)] text-white">
                다음
              </button>
            )}
          </div>
        ...
        {/* Step 3: Urgency + Title */}
        {step === 3 && (
          <div>
            <h2 className="text-[18px] font-bold mb-1">긴급도와 제목을 입력해주세요</h2>
        ...
        {step === 4 && (
          <div>
            ...
            <p className="text-[13px] text-[var(--sub)] mb-5">{title}</p>
            <Card className="p-4">
              <textarea
                value={description}
            ...
            <button
              onClick={() => setStep(5)}
              className="w-full mt-6 py-3.5 rounded-xl text-[15px] font-semibold bg-[var(--blue)] text-white"
            >
              다음
            </button>
          </div>
        ...
        {/* Step 5: Worker + Cost + Submit */}
        {step === 5 && (
          <div>
            <h2 className="text-[18px] font-bold mb-1">작업 정보를 입력해주세요</h2>
            <p className="text-[13px] text-[var(--sub)] mb-5">선택사항입니다. 건너뛰어도 됩니다.</p>

            <p className="text-[13px] font-semibold mb-2">작업자 배정</p>
            <div className="flex flex-col gap-2 mb-5">
              <button
                onClick={() => setWorkerId('')}
                className={`text-left px-4 py-3 rounded-xl border transition-colors ${
                  !workerId ? 'border-[var(--blue)] bg-[var(--blue-light)]' : 'border-[var(--border)] bg-[var(--card)]'
                }`}
              >
                <span className="text-[14px] font-medium">미배정</span>
              </button>
              {workers.map(w => (
                <button
                  key={w.id}
                  ...
                  className={`text-left px-4 py-3 rounded-xl border transition-colors ${
                    workerId === w.id ? 'border-[var(--blue)] bg-[var(--blue-light)]' : 'border-[var(--border)] bg-[var(--card)]'
                  }`}
                >
                  <span className="text-[14px] font-medium">{w.name}</span>
                  <span className="text-[12px] text-[var(--sub)] ml-2">{w.specialty}</span>
                </button>
              ))}
            </div>

            <p className="text-[13px] font-semibold mb-2">예상 비용</p>
            <input
              type="number"
              value={cost}
            ...
            {/* Summary Card */}
            <Card className="mt-5 p-4 bg-[var(--blue-light)]">
              <p className="text-[12px] font-bold text-[var(--blue)] mb-2">등록 요약</p>
              <div className="grid grid-cols-2 gap-1 text-[12px]">
                <span className="text-[var(--sub)]">지점/호실</span>
                <span className="font-medium text-right">{house} {room}</span>
                <span className="text-[var(--sub)]">카테고리</span>
                <span className="font-medium text-right">{category}</span>
                <span className="text-[var(--sub)]">긴급도</span>
                <span className="font-medium text-right">{urgency}</span>
                <span className="text-[var(--sub)]">제목</span>
                <span className="font-medium text-right truncate">{title}</span>
                {workerId && <>
                  <span className="text-[var(--sub)]">작업자</span>
                  <span className="font-medium text-right">{workers.find(w => w.id === workerId)?.name}</span>
                </>}
                {cost && <>
                  <span className="text-[var(--sub)]">예상비용</span>
                  <span className="font-medium text-right">{Number(cost).toLocaleString()}원</span>
                </>}
              </div>
            </Card>
