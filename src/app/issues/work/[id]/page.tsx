...
export default function WorkDetailPage() {
  ...
  return (
    ...
    >
      ...
      {/* 카드 2: 상태 */}
      <div style={CARD}>
        <span style={SECTION_LABEL}>상태</span>
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            onClick={() => toggleDone('N')}
            style={statusBtnStyle(!isDone, '#191f28')}
          >
            미완료
          </button>
          <button
            onClick={() => toggleDone('Y')}
            style={statusBtnStyle(isDone, GREEN)}
          >
            완료
          </button>
        </div>
        {isDone && work.완료일 && (
          <p style={{ fontSize: 11, color: GRAY400, marginTop: 8, textAlign: 'right' }}>
            완료일 {work.완료일}
          </p>
        )}
      </div>
